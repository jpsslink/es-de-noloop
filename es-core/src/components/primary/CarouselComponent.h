...
CarouselComponent<T>::CarouselComponent()
    : IList<CarouselEntry, T> {IList<CarouselEntry, T>::LIST_SCROLL_STYLE_SLOW,
                               (std::is_same_v<T, SystemData*> ?
                                    ...
                                    // Custom patch: disable wraparound looping for the gamelist
                                    // (game) carousel so it stops at the first/last item instead
                                    // of cycling back around.
                                    ListLoopType::LIST_NEVER_LOOP)}
